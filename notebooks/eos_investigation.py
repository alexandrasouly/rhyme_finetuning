...
MODEL = "../../output/supervised_eos2/"
model = transformers.AutoModelForCausalLM.from_pretrained(
    MODEL)
tokenizer = transformers.AutoTokenizer.from_pretrained("gpt2")
# %%
input_ids = tokenizer('She walks in beauty, like the night\n')['input_ids']

result = model.generate(input_ids=t.tensor(input_ids).unsqueeze(
    dim=0), output_scores=True, return_dict_in_generate=True, early_stopping=True)

sequences = result.sequences.squeeze(dim=0)
print(tokenizer.decode(sequences))
probs = t.softmax(t.stack(result.scores, dim=0), dim=-1)

# the output sequence without the prompt
output_sequences = result.sequences.squeeze(dim=0)[len(input_ids):]
